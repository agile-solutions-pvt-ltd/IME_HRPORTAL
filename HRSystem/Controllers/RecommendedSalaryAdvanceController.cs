...
using RecommendedSalaryAdvance;

namespace HRSystem.Controllers
{
    public class RecommendedSalaryAdvanceController : Controller
    {
        ...
        public RecommendedSalaryAdvanceController(IMapper mapper)
        {
            _mapper = mapper;
        ...
        private recommendedsalaryadvance_PortClient Recommendedsalaryadvance_PortClientService()
        {
            ...
            var integrationService = config.Integration_Services
                .Where(x => x.Integration_Type == "Recommended_Salary" && x.Company_Name == config.Default_Config.Company_Name)
                .FirstOrDefault();
            ...
            var client = new recommendedsalaryadvance_PortClient(basicHttpBinding, endpoint);
        ...
        {
            List<recommendedsalaryadvance> result = new List<recommendedsalaryadvance>();
            try
            {
                recommendedsalaryadvance_Filter[] filter = new recommendedsalaryadvance_Filter[0];
                result = Recommendedsalaryadvance_PortClientService()
        ...
        public IActionResult VerifyRequest(string salaryAdvanceNo)
        {
        ...
        [HttpPost]
        public IActionResult VerifyRequest(PostedSalaryAdvanceViewModel vmObj)
        {
            try
            {
                ...
                if (ModelState.IsValid)
                {
                    ...
                    if (result != null)
                    {
                        ...
                        .VerifysalaryadvancewebAsync(result.No)
                        .GetAwaiter()
                        .GetResult()
                        .return_value;
                        ...
                        if (postResult == 200)
                        {
                            TempData["Notify"] = JsonConvert.SerializeObject(new Notify { title = "Recommend Salary Advance", text = "Salary advance verified successfully.", type = "success" });
                            return RedirectToAction(nameof(Index));
                        }
                        else
                        {
                            TempData["Notify"] = JsonConvert.SerializeObject(new Notify { title = "Recommend Salary Advance", text = "Updating salary advance succeeded but posting verification failed.", type = "error" });
                        }
                    }
                    else
                    {
                ...
                {
                    TempData["Notify"] = JsonConvert.SerializeObject(new Notify { title = "Approve Salary Advance", text = "Validation Error. Try Again.", type = "error" });
                }
            }
            catch (Exception ex)
            {
            ...
            return RedirectToAction(nameof(VerifyRequest), new { salaryAdvanceNo = vmObj.No });
        }
