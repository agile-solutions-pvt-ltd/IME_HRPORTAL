...
using RecommendedTravelOrder;

namespace HRSystem.Controllers
{
    public class RecommendedTravelOrderController : Controller
    {
        ...
        public RecommendedTravelOrderController(IMapper mapper)
        {
            _mapper = mapper;
        ...
        private recommendedtravelorder_PortClient Recommendedtravelorder_PortClientService()
        {
            ...
            var integrationService = config.Integration_Services
                .Where(x => x.Integration_Type == "Recommended_Travel" && x.Company_Name == config.Default_Config.Company_Name)
                .FirstOrDefault();
            ...
            var client = new recommendedtravelorder_PortClient(basicHttpBinding, endpoint);
        ...
        {
            List<recommendedtravelorder> result = new List<recommendedtravelorder>();
            try
            {
                recommendedtravelorder_Filter[] filter = new recommendedtravelorder_Filter[0];
                result = Recommendedtravelorder_PortClientService()
        ...
        [HttpPost]
        public IActionResult ApproveRequest(PostedTravelOrderViewModel vmObj)
        {
            try
            {
                ...
                if (ModelState.IsValid)
                {
                    ...
                    if (result != null)
                    {
                        ...
                        .ApprovetravelorderwebAsync(result.Travel_Order_No)
                        .GetAwaiter()
                        .GetResult()
                        .return_value;
                        ...
                        if (postResult == 200)
                        {
                            TempData["Notify"] = JsonConvert.SerializeObject(new Notify { title = "Recommend Travel Order", text = "Travel Request approved successfully.", type = "success" });
                            return RedirectToAction(nameof(Index));
                        }
                        else
                        {
                            TempData["Notify"] = JsonConvert.SerializeObject(new Notify { title = "Recommend Travel Order", text = "Updating travel order succeeded but posting approval failed.", type = "error" });
                        }
                    }
                    else
                    {
            ...
            return RedirectToAction(nameof(ApproveRequest), new { travelOrderNo = vmObj.Travel_Order_No });
        }
