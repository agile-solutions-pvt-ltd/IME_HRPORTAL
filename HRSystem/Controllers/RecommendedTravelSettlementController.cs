...
using RecommendedTravelVoucher;

namespace HRSystem.Controllers
{
    [Authorize]
    public class RecommendedTravelSettlementController : Controller
    {
        ...
        public RecommendedTravelSettlementController(IMapper mapper)
        {
            _mapper = mapper;
        ...
        private recommendedtravelvoucher_PortClient Recommendedtravelvoucher_PortClientService()
        {
            ...
            var integrationService = config.Integration_Services
                .Where(x => x.Integration_Type == "Recommended_Travel_Voucher" && x.Company_Name == config.Default_Config.Company_Name)
                .FirstOrDefault();
            ...
            var client = new recommendedtravelvoucher_PortClient(basicHttpBinding, endpoint);
        ...
        {
            List<recommendedtravelvoucher> recommendedtravelvouchers = new List<recommendedtravelvoucher>();
            try
            {
                recommendedtravelvoucher_Filter[] filter = new recommendedtravelvoucher_Filter[0];
                recommendedtravelvouchers = Recommendedtravelvoucher_PortClientService()
            ...
            return View(recommendedtravelvouchers);
        }

        public IActionResult VerifyRequest(string travelOrderNo)
        {
            ...
            try
            {
                if (ModelState.IsValid)
                {
                    ...
                    if (result != null)
                    {
                        ...
                        if (postResult == 200)
                        {
                            TempData["Notify"] = JsonConvert.SerializeObject(new Notify { title = "Verify Travel Settlement", text = "Travel Settlement verified successfully.", type = "success" });
                            return RedirectToAction(nameof(Index));
                        }
                        else
                        {
                            TempData["Notify"] = JsonConvert.SerializeObject(new Notify { title = "Verify Travel Settlement", text = "Updating travel settlement succeeded but posting verification failed.", type = "error" });
                        }
                    }
                    else
                    {
                        TempData["Notify"] = JsonConvert.SerializeObject(new Notify { title = "Verify Travel Settlement", text = "Updating travel settlement fields failed.", type = "error" });
                    }
                }
                else
                {
                    TempData["Notify"] = JsonConvert.SerializeObject(new Notify { title = "Verify Travel Settlement", text = "Validation Error. Try Again.", type = "error" });
                }
            }
            catch (Exception ex)
            {
